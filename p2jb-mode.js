...
(async function () {
    try {
        ...
        async function p2jb_bootstrap_payload_manager_after_complete() {
            ...
            const PM_NAMES = [
                "Payload_Manager_v0.1.elf",
                "Payload_Manager.elf",
                "payload_manager.elf",
                "pldmgr.elf"
            ];

            const blog = async (msg) => { await ulog("pm_bootstrap: " + msg); };

            function htons16(v) {
                return ((v & 0xff) << 8) | ((v >> 8) & 0xff);
            }

            function file_exists_local(path) {
                const st = malloc(0x200n);
                const r = syscall(SYSCALL.stat, alloc_string(path), st);
                return r === 0n;
            }

            function read_file_to_buffer_local(path) {
                const st = malloc(0x200n);
                ...
                const fd = syscall(SYSCALL.open, alloc_string(path), 0n, 0n);
            ...
            async function send_buffer_to_elfldr(path, buf, size) {
                ...
                for (let attempt = 1; attempt <= MAX_CONNECT_RETRIES; attempt++) {
                    ...
                    const sockaddr = make_localhost_sockaddr(PM_PORT);
                    const cr = syscall(SYSCALL.connect, sock, sockaddr, 16n);
                    if (cr === 0n) {
                        await blog("connected to 127.0.0.1:" + PM_PORT + " for " + path);
                        ...
                        await blog("sent " + wr.toString() + " bytes: " + path);
                ...
                throw new Error("connect 127.0.0.1:" + PM_PORT + " failed for " + path + " last=" + toHex(last));
            }

            function candidate_paths() {
                const dirs = [
                    "/data/ps5_payload_manager",
                    "/data/ps5_autoloader",
                    "/data/pldmgr",
                    "/data/pldmgr/payloads"
                ];
                for (let i = 0; i <= 7; i++) {
                    dirs.push("/mnt/usb" + i + "/ps5_payload_manager");
                    dirs.push("/mnt/usb" + i + "/ps5_autoloader");
                    dirs.push("/mnt/usb" + i + "/pldmgr");
                    dirs.push("/mnt/usb" + i + "/pldmgr/payloads");
                }
                const out = [];
                for (const d of dirs) {
                    for (const n of PM_NAMES) out.push(d + "/" + n);
                }
                return out;
            }
            ...
            for (const p of candidate_paths()) {
                if (file_exists_local(p)) { pm_path = p; break; }
            }
            if (!pm_path) {
                await blog("Payload Manager ELF not found; skipping");
                send_notification("Payload Manager\nELF not found");
                return;
            }
        ...
        try {
            await p2jb_bootstrap_payload_manager_after_complete();
        } catch (e) {
            await ulog("pm_bootstrap: failed: " + e.message + " (jailbreak unaffected)");
            try { send_notification("Payload Manager bootstrap FAILED\n" + e.message); } catch (_) { }
        }
