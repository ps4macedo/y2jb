...
(async function () {
    try {
        ...
        async function p2jb_autoload_after_complete() {
            const AUTOLOAD_DIRNAME = "ps5_autoloader";
            const AUTOLOAD_PORT = 9021;
            const MAX_CONNECT_RETRIES = 50;
            const CONNECT_RETRY_MS = 200;

            const alog = async (msg) => { await ulog("autoload: " + msg); };

            function htons16(v) {
                return ((v & 0xff) << 8) | ((v >> 8) & 0xff);
            }

            function read_string_from_buffer(addr, len) {
                let out = "";
                for (let i = 0; i < len; i++) {
                    const c = Number(read8(addr + BigInt(i)) & 0xffn);
                    if (c === 0) break;
            ...
            function read_file_text(path) {
                const fd = syscall(SYSCALL.open, alloc_string(path), O_RDONLY, 0n);
                if (fd === 0xffffffffffffffffn || fd < 0n) {
                    throw new Error("open failed: " + path + " ret=" + toHex(fd));
                }
                const buf = malloc(4096n);
                const chunks = [];
                try {
                    while (true) {
                        const n = syscall(SYSCALL.read, fd, buf, 4096n);
                        if (n === 0xffffffffffffffffn || n < 0n) {
                            throw new Error("read failed: " + path + " ret=" + toHex(n));
                        }
                        if (n === 0n) break;
                        chunks.push(read_string_from_buffer(buf, Number(n)));
                    }
                } finally {
            ...
            function file_exists(path) {
                const st = malloc(0x200n);
                const r = syscall(SYSCALL.stat, alloc_string(path), st);
                return r === 0n;
            }

            function read_file_to_buffer(path) {
                const st = malloc(0x200n);
                const sr = syscall(SYSCALL.stat, alloc_string(path), st);
                if (sr !== 0n) throw new Error("stat failed: " + path + " ret=" + toHex(sr));
                const size = Number(read64(st + 72n));
                if (size <= 0) throw new Error("invalid file size: " + path + " size=" + size);
                const fd = syscall(SYSCALL.open, alloc_string(path), O_RDONLY, 0n);
                if (fd === 0xffffffffffffffffn || fd < 0n) {
                    throw new Error("open failed: " + path + " ret=" + toHex(fd));
                }
                const buf = malloc(BigInt(size));
                try {
                    const n = syscall(SYSCALL.read, fd, buf, BigInt(size));
                    if (n === 0xffffffffffffffffn || n < 0n) {
                        throw new Error("read failed: " + path + " ret=" + toHex(n));
                    }
                    if (Number(n) !== size) {
                        throw new Error("short read: " + path + " got=" + Number(n) + " expected=" + size);
                    }
                } finally {
                    syscall(SYSCALL.close, fd);
                }
            ...
            function get_title_id_safe() {
                ...
                try {
                    ...
                    const r = syscall(SYSCALL.sysctl, mib, 4n, app_info, oldlen, 0n, 0n);
                    if (r === 0n) return read_string_from_buffer(app_info + 0x10n, 16);
                } catch (_) { }
                return "";
            }

            function make_localhost_sockaddr(port) {
                const sockaddr = malloc(16n);
                for (let i = 0n; i < 16n; i++) write8(sockaddr + i, 0n);
                write8(sockaddr + 0n, 16n);
                write8(sockaddr + 1n, AF_INET);
                write16(sockaddr + 2n, BigInt(htons16(port)));
            ...
            async function send_buffer_to_elfldr(path, buf, size) {
                let last = 0n;
                for (let attempt = 1; attempt <= MAX_CONNECT_RETRIES; attempt++) {
                    const sock = syscall(SYSCALL.socket, AF_INET, SOCK_STREAM, 0n);
                    if (sock === 0xffffffffffffffffn || sock < 0n) {
                        last = sock;
                        await js_sleep(CONNECT_RETRY_MS);
                        continue;
                    }
                    const enable = malloc(4n);
                    write32(enable, 1n);
                    syscall(SYSCALL.setsockopt, sock, SOL_SOCKET, SO_REUSEADDR, enable, 4n);
                    const sockaddr = make_localhost_sockaddr(AUTOLOAD_PORT);
                    const cr = syscall(SYSCALL.connect, sock, sockaddr, 16n);
                    if (cr === 0n) {
                        await alog("connected to 127.0.0.1:" + AUTOLOAD_PORT + " for " + path);
                        const wr = syscall(SYSCALL.write, sock, buf, BigInt(size));
                        syscall(SYSCALL.close, sock);
                        if (wr === 0xffffffffffffffffn || wr < 0n) {
                            throw new Error("write failed for " + path + " ret=" + toHex(wr));
                        }
                        await alog("sent " + wr.toString() + " bytes: " + path);
                        send_notification("autoload sent\n" + path);
                        return true;
                    }
                    last = cr;
                    syscall(SYSCALL.close, sock);
                ...
                throw new Error("connect 127.0.0.1:" + AUTOLOAD_PORT + " failed for " + path + " last=" + toHex(last));
            }

            async function send_payload_file(path) {
                const f = read_file_to_buffer(path);
                await alog("sending " + f.size + " bytes: " + path);
                await send_buffer_to_elfldr(path, f.buffer, f.size);
            }

            function run_js_file(path) {
                const code = read_file_text(path);
                const fn = new Function(code);
                fn();
            }

            async function kill_youtube_after_autoload(delay_ms) {
                await alog("closing YouTube in " + delay_ms + " ms");
            ...
            const title = get_title_id_safe();
            const paths = [];
            if (title.length > 0) {
                for (let i = 0; i <= 7; i++) paths.push("/mnt/usb" + i + "/" + AUTOLOAD_DIRNAME + "_" + title + "/autoload.txt");
                paths.push("/data/" + AUTOLOAD_DIRNAME + "_" + title + "/autoload.txt");
            }
            for (let i = 0; i <= 7; i++) paths.push("/mnt/usb" + i + "/" + AUTOLOAD_DIRNAME + "/autoload.txt");
            paths.push("/data/" + AUTOLOAD_DIRNAME + "/autoload.txt");
            if (title.length > 0) {
                paths.push("/mnt/sandbox/" + title + "_000/download0/cache/splash_screen/aHR0cHM6Ly93d3cueW91dHViZS5jb20vdHY=/" + AUTOLOAD_DIRNAME + "/autoload.txt");
            }

            await alog("starting after p2jb complete");
            await alog("elfldr target 127.0.0.1:" + AUTOLOAD_PORT);
            if (title.length > 0) await alog("title_id=" + title);

            let cfg = null;
            for (const p of paths) {
                if (file_exists(p)) { cfg = p; break; }
            }
            if (!cfg) {
                await alog("no autoload.txt found; skipping");
                send_notification("autoload\nno autoload.txt found");
                return;
            }

            await alog("loading list " + cfg);
            send_notification("autoload list found\n" + cfg);
            const dir = cfg.substring(0, cfg.lastIndexOf('/') + 1);
            const lines = read_file_text(cfg).split('\n');
            let close_after = false;
            let close_delay = 500;
            let processed = 0;
            ...
            for (const raw of lines) {
                ...
                await alog("line: " + line);

                if (line.startsWith('!')) {
                    const arg = line.substring(1).trim();
                    const ms = parseInt(arg, 10);
                    if (!isNaN(ms) && ms > 0) {
                        await alog("delay " + ms + " ms");
                        await js_sleep(ms);
                    } else {
                        await alog("invalid delay ignored: " + arg);
                    }
                    continue;
                }

                if (line.startsWith('@')) {
                    const msg = line.substring(1).trim().replace(/\\n/g, '\n');
                    send_notification(msg);
                    await alog("notification: " + msg.replace(/\n/g, " | "));
                    continue;
                }

                if (line === ':close' || line === ':close_youtube') {
                    close_after = true;
                    await alog("close YouTube requested after autoload");
                    continue;
                }

                if (line.startsWith(':close=')) {
                    const ms = parseInt(line.substring(7).trim(), 10);
                    close_after = true;
                    if (!isNaN(ms) && ms >= 0) close_delay = ms;
                    await alog("close YouTube requested after autoload, delay=" + close_delay);
                    continue;
                }

                const full = line.startsWith('/') ? line : dir + line;
                if (!file_exists(full)) {
                    await alog("ERROR file not found: " + full);
                    send_notification("autoload error\nfile not found:\n" + full);
                    continue;
                }

                if (line.endsWith('.elf') || line.endsWith('.bin')) {
                    await send_payload_file(full);
                    processed++;
                    continue;
                }

                if (line.endsWith('.js')) {
                    await alog("executing js " + full);
                    run_js_file(full);
                    processed++;
                    continue;
                }

                await alog("unsupported entry ignored: " + line);
            }

            await alog("complete, processed=" + processed);
            send_notification("autoload complete\nprocessed=" + processed);
            if (close_after) await kill_youtube_after_autoload(close_delay);
        }
        ...
        try {
            await p2jb_autoload_after_complete();
        } catch (e) {
            await ulog("autoload: failed: " + e.message + " (jailbreak unaffected)");
            try { send_notification("autoload FAILED\n" + e.message); } catch (_) { }
        }
